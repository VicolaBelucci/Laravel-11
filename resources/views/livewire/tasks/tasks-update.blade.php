    ...
    <div class="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center p-4">
        ...
        <div class="bg-white p-6 rounded-lg shadow-lg max-h-full overflow-y-auto" style="min-width: 600px;">
            ...
            <div class="flex justify-between items-center mb-3">
                <h2 class="text-xl font-bold">{{$dataToModal ? 'Mostrar' : 'Editar'}} Tarefa</h2>
                <!-- Botão de Fechar -->
            ...
            <form wire:submit='update'>
                ...
                <div class="mb-4">
                    <label for="form.expirationDate" class="block text-gray-700 text-sm font-bold mb-2">Data de
                        Expiração:</label>
                    <input type="date" id="form.expirationDate" wire:model.live="form.expirationDate" @if($dataToModal) readonly @endif
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                ...
                <fieldset class="mb-4 flex justify-between">
                    ...
                    <div class="flex items-center mb-1">
                        <input type="radio" name="status" id="pending" wire:model.live="form.status" value="pending" class="mr-2" checked @if($dataToModal) disabled @endif>
                        <label for="pending">Pendente</label>
                    </div>
                    <div class="flex items-center mb-1">
                        <input type="radio" name="status" id="inProgress" wire:model.live="form.status" value="in_progress" class="mr-2" @if($dataToModal) disabled @endif>
                        <label for="inProgress">Em Progresso</label>
                    </div>
                    <div class="flex items-center mb-1">
                        <input type="radio" name="status" id="completed" wire:model.live="form.status" value="completed" class="mr-2" @if($dataToModal) disabled @endif>
                ...
                <div class="mb-4 linha-sub-tarefa">
                    ...
                    <div class="flex-col">
                        ...
                        <select id="subTarefa" wire:model.live="subTarefa" class="block appearance-none bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline" @if($dataToModal) disabled @endif>
                    ...
                    @if($subTarefa == 1)
                        <div class="flex-col">
                            <label for="relatedTasks" class=" text-gray-700 text-sm font-bold mb-2">Tarefas
                                Relacionadas:</label>
                            <select id="relatedTasks" wire:model.live="form.related_id" class="block appearance-none bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline" @if($dataToModal) disabled @endif>
